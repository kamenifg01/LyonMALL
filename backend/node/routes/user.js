...
router.post("/signup", (req, res) => {
  ...
  connection.query(query, [user.email], (err, result) => {
    if (!err) {
      if (result.length <= 0) {
        query =
          "insert into user(name,contactNumber,email,password,role) values(?,?,?,?,'user');";
        connection.query(
          query,
    ...
    if (!err) {
      if (results.length <= 0) {
        ...
        var mailOptions = {
          ...
          html:
            "<p><br> Your login details for Our Ecommerce system</br> <br><b>Email: </b>" +
            results[0].email +
            "<br><b>Password: </b>" +
            results[0].password +
...
router.get("/get", auth.authenticationToken,checkRole.checkRole,(req, res) => {
  var query =
    "select id,name,email,contactNumber from user where role='user'";
